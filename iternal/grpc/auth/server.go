...
func Register(gRPC *grpc.Server, auth Auth) {
	grpcv1.RegisterAuthServer(gRPC, &serverAPI{auth: auth})
}
...
func validateLogin(req *grpcv1.LoginRequest) error {
	if req.GetEmail() == "" {
...
func validateRegister(req *grpcv1.RegisterRequest) error {
	if req.GetEmail() == "" {
		return status.Error(codes.InvalidArgument, "email is required")
	}
	if req.GetPassword() == "" {
...
func validateIsAdmin(req *grpcv1.IsAdminRequest) error {
